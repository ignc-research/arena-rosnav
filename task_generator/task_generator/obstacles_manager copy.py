...
class ObstaclesManager:
    ...
    def register_obstacles(self, num_obstacles: int, model_yaml_file_path: str, start_pos: list=[] , vertices:np.ndarray=np.array([]), type_obstacle:str='dynamic'):
    ...
    def register_static_obstacle_polygon(self, vertices: np.ndarray):
        ...
        model_path, start_pos = self._generate_static_obstacle_polygon_yaml(
            vertices)
        self.register_obstacles(1, model_path, start_pos, vertices, type_obstacle='static')
        os.remove(model_path)
    ...
    # def update_freespace_indices(self, free_space_indices_new):
    #     self._free_space_indices=free_space_indices_new
