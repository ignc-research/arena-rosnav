...
/** \brief Fade Export Datastructure
 *
 * This simple struct is there to get data out of Fade easily and
 * memory efficiently. The source code of this class is deliberately
 * included in the header file so that users can take over the code
...
struct CLASS_DECLSPEC FadeExport
{
	...
	/// Write an *.obj file (for MeshLab and many other viewers)
	...
	// DATA
	int numCustomIndices; ///<number of custom indices (0 or numPoints)
	int numTriangles;///< number of triangles
	int numPoints; ///< number of points
	double* aCoords; ///< Cartesian coordinates (dim*numPoints)
	int* aCustomIndices; ///< 1 custom index per point (numCustomIndices)
	int* aTriangles; ///< 3 vertex-indices per triangle (3*numTriangles)
	int dim; ///< Dimension
};
