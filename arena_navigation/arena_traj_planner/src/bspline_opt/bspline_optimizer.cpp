...
std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
{
    ...
    if (segment_ids.size() == 0)
    {
      ...
      ROS_WARN_STREAM("Segment 2: return in advance********* ");
      return blank_ret;
    }
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        if (got_intersection_id >= 0)
        {
          ...
          if (length > 1e-5)
          {
            ...
            {
              occ = grid_map_->getFusedDynamicInflateOccupancy((a / length) * intersection_point + (1 - a / length) * init_points.col(j));
...
{
     ROS_WARN_STREAM("Distinct Traj 0:seg_size :"<<segments.size());
    ...
    ROS_WARN_STREAM("Distinct Traj 1:seg_upbound :"<<seg_upbound);
        ...
        if (RichInfoSegs[i].first.size > 1)
        {
            ...
            for (int j = 0; j < RichInfoSegs[i].first.size - 1; j++)
            {
                ...
                for (double a = 1; a > 0; a -= step_size)
                {
                    ...
                    if (grid_map_->getFusedDynamicInflateOccupancy(pt))
                    {
                        occ_start_id = j;
                        occ_start_pt = pt;
            ...
            exit_multi_loop1:;// search for occ_end_pt
            ...
            // double check(the first or the last control points should not in obstacle)
            ...
            for (int j = occ_start_id; j <= occ_end_id; j++)
            {
                ...
                if (grid_map_->getFusedDynamicInflateOccupancy(base_pt_reverse)) // Search outward.
                {
                    ...
                    for (; l <= l_upbound; l += RESOLUTION)
                    {
                        ...
                        if (!grid_map_->getFusedDynamicInflateOccupancy(base_pt_temp))
                        {
...
bool BsplineOptimizer::rebound_optimize(double &final_cost)
{
    ...
    do
    {
        ...
        /* ---------- optimize ---------- */
        t1 = ros::Time::now();
        // variable_num_: num_varibale,
        // q: The array of variables, 
        // &final_cost: The pointer to the variable that receives the final value of the objective function
        // BsplineOptimizer::costFunctionRebound :The callback function to provide function and gradient evaluations given a current values of variables
        // NULL: The callback function to provide values of the upperbound of the stepsize to search 
            ...
            result == lbfgs::LBFGS_STOP)
        {
            ...
            for (double t = tm; t < tmp ; t += t_step)//(double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
            {   
                flag_occ = grid_map_->getFusedDynamicInflateOccupancy(traj.evaluateDeBoorT(t));
                if (flag_occ)
                {
                    ...
                    if (t <= bspline_interval_) // First 3 control points in obstacles!
                    {
                        ...
                        ROS_WARN("First 3 control points in obstacles! return false, t=%f, t_step=%f", t,t_step);
                        return false;
                    }
        ...
        Eigen::Vector2d start_end_vec = traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm);
        Eigen::Vector2d offset_xy(-start_end_vec(0), start_end_vec(1), 0);
        offset_xy.normalize();
        Eigen::Vector2d offset_z = start_end_vec.cross(offset_xy);
        offset_z.normalize();
...
bool BsplineOptimizer::refine_optimize()
{
    ...
    do
    {
      ...
      {
        if (grid_map_->getFusedDynamicInflateOccupancy(traj.evaluateDeBoorT(t)))
        {
...
void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
{
    ...
    // Eigen::MatrixXd g_mov_objs = Eigen::MatrixXd::Zero(2, cps_.size);
    //Eigen::MatrixXd g_swarm = Eigen::MatrixXd::Zero(2, cps_.size);
...
// collision check for distanceCostRebound
bool BsplineOptimizer::check_collision_and_rebound(void)
{
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      bool occ = grid_map_->getFusedDynamicInflateOccupancy(cps_.points.col(i));
      ...
      if (occ)
      {
        ...
        for (j = i - 1; j >= 0; --j)
        {
          occ = grid_map_->getFusedDynamicInflateOccupancy(cps_.points.col(j));
          if (!occ)
          {
            in_id = j;
            break;
        ...
        for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
        {
          ...
          if (got_intersection_id >= 0)
          {
            ...
            if (length > 1e-5)
            {
              ...
              for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
              {
                bool occ = grid_map_->getFusedDynamicInflateOccupancy((a / length) * intersection_point + (1 - a / length) * cps_.points.col(j));
...
/* optimization cost function: refine */
