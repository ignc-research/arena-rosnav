...
def plot_bars(data):
    ...
    for metric in metrics:
        ...
        plt.ylabel('{}'.format(label_mapping[metric]), fontsize=16)
        plt.yticks(fontsize="15")
        # plt.title("{} over No. Obstacles".format(label_mapping[metric]), fontweight='bold', fontsize=16)
        ax.grid('on')
        # plt.legend(loc='upper left', bbox_to_anchor=(1.05, 1.02), fontsize="14")
        plt.savefig(path+'/{0}.png'.format(metric), bbox_inches="tight")
