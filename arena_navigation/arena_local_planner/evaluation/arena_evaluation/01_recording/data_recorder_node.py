...
class recorder():
    def __init__(self) -> None:
        ...
        self.dir_path = os.path.join(rospkg.RosPack().get_path('arena_evaluation'), '01_recording')
        if self.record_only_planner:
            with open(self.dir_path + "/{0}_{1}_{2}.csv".format(self.local_planner, self.scenario, self.now), "w+",
                      newline="") as file:
                writer = csv.writer(file, delimiter=',')
                header = [["episode", "time", "laser_scan", "robot_lin_vel_x", "robot_lin_vel_y", "robot_ang_vel",
                           "robot_orientation", "robot_pos_x", "robot_pos_y", "action"]]
                writer.writerows(header)
                file.close()
        else:
            with open(self.dir_path + "/{0}_{1}_{2}_{3}.csv".format(self.local_planner, self.waypoint_generator,
                                                                    self.scenario, self.now), "w+", newline="") as file:
                writer = csv.writer(file, delimiter=',')
                header = [["episode", "time", "laser_scan", "robot_lin_vel_x", "robot_lin_vel_y", "robot_ang_vel",
                           "robot_orientation", "robot_pos_x", "robot_pos_y", "action"]]
                writer.writerows(header)
    ...
    def action_callback(self, msg_action: Twist):  # variables will be written to csv whenever an action is published
        ...
        if self.episode == self.max_episodes:
            subprocess.call(["tmux", "kill-server"])  # apt-get install psmisc necessary
            sys.exit()

    def addData(self, data: np.array):  # add new row to the csv file
        with open(self.dir_path + "/{0}_{1}_{2}.csv".format(self.local_planner, self.scenario, self.now), "a+",
                  newline="") as file:
