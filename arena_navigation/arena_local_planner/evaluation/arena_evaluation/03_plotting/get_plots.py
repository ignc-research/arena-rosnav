...
def filter_paths(paths: dict):
    # remove elements at the beginning of the path from the previous episode
    paths_filtered = copy.copy(paths)
    for i, path in enumerate(paths.values()):
        ...
        del_x = e_t[0] - e_t_1[0]
        del_y = e_t[1] - e_t_1[1]
        while del_x ** 2 + del_y ** 2 < 2 and j <= 11:
            j += 1
            e_t = path[j]
            e_t_1 = path[j - 1]
            del_x = e_t[0] - e_t_1[0]
        ...
        if j != 11:
            paths_filtered[str(i)] = paths_filtered[str(i)][j:]
...
class plotter():
    def __init__(self):
        self.dir_path = os.path.dirname(
            os.path.abspath(__file__))  # get path for current file, does not work if os.chdir() was used
        self.data_dir = os.path.dirname(
            self.dir_path) + "/02_evaluation"  # parent_directory_path + directory name where csv files are located
    ...
    def grab_data(self):  # move data.json from 02_evaluation into 03_plotting/data
        ...
        try:
            self.most_recent_grab = sorted(files)[-1].split("/")[
                -1]  # get last in order JSON data file from /02_evaluation
        except:
            self.most_recent_grab = None
        for file in files:
            file_name = file.split("/")[-1]
            os.rename(self.data_dir + "/" + file_name,
                      self.dir_path + "/data/" + file_name)  # move file from dir_path to data folder
    ...
    def load_data(self):
        ...
        try:
            ...
            with open(self.dir_path + '/data/{}'.format(self.data_file_name)) as json_file:
                self.data = json.load(json_file)
            print(
                "INFO: Specified data file could not be found. Using most recent data file from data directory by default.")
            print("INFO: Dataset {} loaded.".format(self.data_file_name))
                ...
                for obstacle_number in self.obstacle_numbers:
                    ...
                    for key in sorted(obs_keys):
                        ...
                        for i, episode in enumerate(episodes):
                            ...
                            x = x[
                                1:-1]  # NOTE: sometimes episode is wrongly assigned _> skip first and last coordinates
                            y = y[1:-1]
                            plt.plot(x, y,
                                     "-",
                                     color=self.config["color_scheme"][planner],
                        ...
                        if self.config["plot_collision_zones"]:
                            ...
                            if len(centroids) != 0:
                                ...
                                for i, centroid in enumerate(centroids):
                                    ...
                                    plt.gca().add_patch(
                                        plt.Circle(tuple(to_ros_coords(centroid, img, map_resolution, map_origin)),
                                                   radius=self.config["collision_zone_base_diameter"] * counts[
                                                       i] / map_resolution,
                                                   color=self.config["color_scheme"][planner],
                                                   fill=False))
                    ...
                    if self.config["plot_legend"]:
                        plt.legend(loc=self.config["legend_position"], fontsize='large')
                    plt.tight_layout()
                    plt.savefig(
                        self.plot_dir + "/qualitative_plot_{0}_{1}_{2}_{3}".format(map, obstacle_number, velocity,
                                                                                   self.now))

    def get_map(self):  # get map from df file name
        self.map_dir = os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))) + "/simulator_setup/maps"
    ...
    def plot_scenario(self, keys, img, map_resolution, map_origin):
        scenario_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))) + "/simulator_setup/scenarios/eval"
        scenario_dict = {x.split("/")[-1].replace(".json", ""): x for x in
                         sorted(glob.glob("{0}/*.json".format(scenario_dir)))}
        ...
        if "pedsim_agents" in scenario.keys():
            ...
            obstacle_paths = [[dynamic_obstacles[obstacle]["start_pos"][:-1]] + [waypoint[:-1] for waypoint in
                                                                                 dynamic_obstacles[obstacle][
                                                                                     "waypoints"]] for obstacle in
                              dynamic_obstacles]
        ...
        plt.scatter(start_x, start_y, marker=self.config["start_marker"], s=self.config["start_size"] / map_resolution,
                    color=self.config["start_point_color"])
        plt.scatter(goal_x, goal_y, marker=self.config["goal_marker"], s=self.config["goal_size"] / map_resolution,
                    color=self.config["goal_point_color"])
        ...
        for path in obstacle_paths:
            ...
            for i, waypoint in enumerate(path):
                ...
                if i == len(path) - 1:
                    continue
                plt.gca().add_patch(patches.FancyArrowPatch(waypoint, path[i + 1], arrowstyle='<->',
                                                            mutation_scale=self.config["path_arrow_size"],
                                                            color=self.config["path_arrow_color"]))
...
def to_ros_coords(coords, img, map_resolution, map_origin):
    if type(coords) == tuple:
        return [img.shape[0] - (y - map_origin[1]) / map_resolution for x, y in coords], [
            img.shape[1] - (x - map_origin[0]) / map_resolution for x, y in coords]
    elif type(coords) == list and (type(coords[0]) == float or type(coords[0]) == int):
        return img.shape[0] - (coords[1] - map_origin[1]) / map_resolution, img.shape[1] - (
                    coords[0] - map_origin[0]) / map_resolution
    elif type(coords) == list:
        return [img.shape[0] - (point[1] - map_origin[1]) / map_resolution for point in coords], [
            img.shape[1] - (point[0] - map_origin[0]) / map_resolution for point in coords]


def transform_waypoints(obstacle_paths, img, map_resolution, map_origin,
                        ped_sim=False):  # only necessary for old scenario.json
    transformed_paths = []
    if ped_sim:
        for path in obstacle_paths:
        ...
        for path in obstacle_paths:
            ...
            waypoint = list(
                to_ros_coords((np.array(path[0]) + np.array(path[1])).tolist(), img, map_resolution, map_origin))
