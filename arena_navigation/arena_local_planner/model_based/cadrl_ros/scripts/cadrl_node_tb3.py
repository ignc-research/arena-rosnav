...
class NN_tb3():
    def __init__(self, veh_name, veh_data, nn, actions):
        ...
        self.pub_pose_marker = rospy.Publisher(
            '~pose_marker', Marker, queue_size=1)
        self.pub_agent_marker = rospy.Publisher(
            '~agent_marker', Marker, queue_size=1)
        self.pub_agent_markers = rospy.Publisher(
            '~agent_markers', MarkerArray, queue_size=1)
        self.pub_path_marker = rospy.Publisher(
            '~path_marker', Marker, queue_size=1)
        ...
        self.sub_mode = rospy.Subscriber(
            '~mode', PlannerMode, self.cbPlannerMode)
        ...
        if self.use_clusters:
            self.sub_clusters = rospy.Subscriber(
                '~clusters', Clusters, self.cbClusters)
        else:
            print("no peds")
    ...
    def visualize_action(self, use_d_min):
        ...
        marker.type = marker.CUBE
        marker.action = marker.ADD
        marker.pose.position = copy.deepcopy(
