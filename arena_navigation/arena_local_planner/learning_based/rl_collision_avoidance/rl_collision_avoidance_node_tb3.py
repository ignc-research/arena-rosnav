...
class NN_tb3():
    ...
    def get_local_goal(self):
        ...
        [goal_x, goal_y] = [self.sub_goal.x,
                            self.sub_goal.y]  # sub goal based on map
    ...
    def cbComputeAction(self, event):
        ...
        self.state = [self.pose.pose.position.x,
                      self.pose.pose.position.y, self.psi]    # x, y, theta
        self.goal = np.asarray(self.get_local_goal())
        self.speed = np.asarray(
            [self.vel.x, self.vel_angular], dtype='float64')
