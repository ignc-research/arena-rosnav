...
class wpEnv(gym.Env):
    ...
    def __init__(self, ns: str, task: ABSTask, robot_yaml_path: str, settings_yaml_path: str, reward_fnc: str, is_action_space_discrete, safe_dist: float = None, goal_radius: float = 0.1, max_steps_per_episode=100, train_mode: bool = True, debug: bool = False):
        ...
        self.agent_action_pub = rospy.Publisher(f'{self.ns_prefix}wp_drl/wp4train', PoseStamped, queue_size=1)        # agent publishes a waypoint on a circle 
        self.circle_pub = rospy.Publisher(f'{self.ns_prefix}zViz', Path, queue_size=1)                               # create the circle on which the wp are placed for visualization
    ...
    def step(self, action):
        ...
        reward, reward_info = self.reward_calculator.get_reward(
            ...
            action=new_action, global_plan=obs_dict['global_plan'], robot_pose=obs_dict['robot_pose'])
...
if __name__ == '__main__':
    ...
    rospy.init_node('wp_gym_env', anonymous=True, disable_signals=False)
    print("start")
