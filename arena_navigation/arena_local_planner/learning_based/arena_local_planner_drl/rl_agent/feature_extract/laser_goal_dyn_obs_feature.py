...
@FEATURE_REGISTRY.register()
class CNN_LaserVAE_Obstalcle_GlobalPlan_Attention(BaseFeaturesExtractor):
    """
    ...
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 64+64+64):
        ...
        # )
        self.global_path_fc = nn.Sequential(
            nn.Linear(CNN_Laser_Obstalcle_GlobalPlan.NUM_POSES_GLOBAL_PLAN*2,64),
            nn.ReLU(),
            nn.Linear(64,32),
            nn.ReLU()
        )
    ...
    def forward(self, observations: th.Tensor) -> th.Tensor:
        ...
        all_features = th.cat((scan_encoded, dynamic_obs_feature,global_path_feature),1)
        attention_coff = th.softmax(self.attention_coff_fc(all_features),dim=1)
        ...
        all_features_with_attention = th.cat((scan_encoded*attention_coff[:,0][:,None],dynamic_obs_feature*attention_coff[:,1][:,None],global_path_feature*attention_coff[:,2][:,None]),axis=1)
#
        return all_features_with_attention
...
@FEATURE_REGISTRY.register()
class CNN_LaserVAE_Obstalcle_GlobalPlan_ONLY_TEST(BaseFeaturesExtractor):
    """
    use pretrained encoder to to encode laser 
