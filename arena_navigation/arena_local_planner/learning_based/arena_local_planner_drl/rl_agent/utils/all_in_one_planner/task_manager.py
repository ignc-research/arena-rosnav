...
class TaskManager:
    ...
    def __init__(self, ns: str, reset_map_interval: int, paths: dict):
        ...
        self._request_new_map = rospy.ServiceProxy("/" + self.ns + "/new_map", std_srvs.srv.SetBool)
    ...
    def reset(self):
        ...
        if self.current_iteration % self.resetMap_interval == 0:
            self.current_iteration = 0
            self._request_new_map(std_srvs.srv.SetBoolRequest())
        self.task.reset()
    ...
    def _update_map(self):
        ...
        service_client_get_map = rospy.ServiceProxy('/' + self.ns + '/static_map', GetMap)
        map_response = service_client_get_map()
        self.obstacles_manager.update_map(map_response.map)
        self.robot_manager.update_map(map_response.map)
