...
class RewardCalculator:
    ...
    def _cal_reward_rule_05(
        ...
        **kwargs
        ...
        if laser_scan.min() > self.safe_dist + 0.33:
            self._reward_distance_global_plan(
                reward_factor=0.2,
                penalty_factor=0.3,
            )
            self._reward_abrupt_vel_change(vel_idx=0, factor=2.5)
            self._reward_abrupt_vel_change(vel_idx=-1, factor=2.0)
            if self.holonomic:
                self._reward_abrupt_vel_change(vel_idx=1, factor=2.5)
