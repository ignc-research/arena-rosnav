...
def write_hyperparameters_json(agent_hyperparams: object, PATHS: dict):
    doc_location = os.path.join(PATHS.get('model'), "hyperparameters.json")

    with open(doc_location, "w", encoding='utf-8') as target:
        json.dump(agent_hyperparams.__dict__, target, ensure_ascii=False, indent=4)
...
def print_hyperparameters_json(PATHS:dict):
    doc_location = os.path.join(PATHS.get('model'), "hyperparameters.json")
