...
class MLP_HUMAN(nn.Module):
    ...
    def __init__(
        ...
        # Policy network
        self.policy_net = nn.Sequential(
            nn.Linear(feature_dim+_RS+_RS+1+33, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, last_layer_dim_pi),
        ...
        # Value network
        self.value_net = nn.Sequential(
            nn.Linear(feature_dim+_RS+_RS+1+33, 64),
            nn.ReLU(),
            nn.Linear(64, 32),
            nn.ReLU(),
    ...
    def forward(self, features: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        ...
        robot_to_guide_state=features[:, 2:2+_RS]
        body_x = self.body_net_laser(features[:, 2+_RS:_L+2+_RS])
        robot_state=features[:, _L+2+_RS:_L+2+_RS+_RS]
        humans_state=features[:, _L+2+_RS:_L+2+_RS+num_humans*human_state_size] 
        robo_obstacles_state=features[:, _L+2+_RS+max_num_humans*human_state_size :_L+2+_RS+max_num_humans*human_state_size+num_robo_obstacles*robo_obstacle_state_size]   
        ...
        features_2=th.cat((features_2, robot_to_guide_state), 1)
