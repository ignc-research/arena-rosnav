...
if __name__ == "__main__":
    ...
    import yaml
    from rl_agent.envs.all_in_one_models.drl.drl_agent import DrlAgent
    from rl_agent.utils.all_in_one_observation_collector import (
        ObservationCollectorAllInOne,
    )
    from geometry_msgs.msg import Twist
                ...
                if body["name"] == "base_footprint":
                    for footprint in body["footprints"]:
                        if footprint["type"] == "circle":
                            robot_radius = footprint.setdefault("radius", 0.3) * 1.05
                        if footprint["radius"]:
    ...
    laser_num_beams, laser_max_range = setup_robot_configuration(
        robot_yaml_path=robot_setting_path, settings_yaml_path=settings_yaml_path
    )
    ns = rospy.get_namespace()
    # PARAM
    # agent = DrlAgent(drl_model_path,'drl_rule_04',ns,True,2)
    agent = DrlAgent(drl_model_path, "drl_rule_04", ns, True, 3)
    ...
    if len(ns):
        ...
        ns_prefix = "/"
    observation_collector = ObservationCollectorAllInOne(
        ns=ns,
        num_lidar_beams=laser_num_beams,
        lidar_range=laser_max_range,
        subgoal_topic_name="waypoint",
        train_mode=False,
        no_wait_for_next_circle=True,
    )
    agent_action_pub = rospy.Publisher(f"{ns_prefix}cmd_vel", Twist, queue_size=1)
    ...
    cb_func_wrapper = lambda _: cb_pub_action(
        agent, observation_collector, agent_action_pub
    )
