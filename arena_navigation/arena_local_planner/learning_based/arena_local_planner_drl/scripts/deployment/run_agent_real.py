...
from arena_navigation.arena_local_planner.learning_based.arena_local_planner_drl.rl_agent.envs.flatland_gym_env_real import FlatlandEnv
...
def make_env(with_ns: bool,
    ...
    def _init():
        ...
        env = FlatlandEnv(
            ...
            goal_radius=0.05, max_steps_per_episode=max_steps_per_episode, train_mode=False, task_mode='manual', PATHS=PATHS, curr_stage=4,
            extended_eval=True)
        if log:
            # eval env
            env = Monitor(
...
if __name__ == "__main__":
    ...
    AGENT = AGENTS.pop(0)
    # AGENT = rospy.get_param("/load_rule") 
    # while len(AGENTS) != 0:
    while True:
        # # AGENT = AGENTS.pop(0) 
        # goal_reached = rospy.get_param("/global_goal_reached")
        # print({goal_reached})
        # if rospy.get_param("/global_goal_reached") == 0:
        ...
        print("[run_agent_real]: created DummyVecEnv")
        if PARAMS['normalize']:
        ...
        # print("[run_agent_real]: created Env")
        done = False
        # load agent
        agent = PPO.load(os.path.join(PATHS['model'], "best_model.zip"), env)
        # print("[run_agent_real]: loaded model")
        obs = env.reset()
        # print("[run_agent_real]: reset env")
        while True:
            # print("performing steps!")
            goal_reached = rospy.get_param("/bool_goal_reached")
            print(f"bool_goal_reached: {goal_reached}")
            if not goal_reached:
                ...
                if done:
                    if args.verbose == '1':
                        ...
                        print(f"Episode finished (finish reason: {done_reason})")
                    env.reset()
                    # wait for next goal
                    rospy.set_param('/bool_goal_reached', True)
                    # break
