...
def get_paths(args: dict, AGENT: str):
    ...
    if HUMANMODE:
        PATHS={
            ...
            'robot_as' : os.path.join(dir, 'configs', 'default_settings_human.yaml'),
            'scenario' : os.path.join(rospkg.RosPack().get_path('simulator_setup'), 'scenarios', args.scenario+'.json'),
            'curriculum': os.path.join(dir, 'configs', 'training_curriculum_human.yaml'),
...
def make_env(with_ns: bool,
    ...
    def _init():
        ...
        env = FlatlandEnv(
            ...
            goal_radius=0.25, max_steps_per_episode=max_steps_per_episode, train_mode=False, task_mode='staged', PATHS=PATHS, curr_stage=1,
            extended_eval=True)
        if log:
            # eval env
            env = Monitor(
