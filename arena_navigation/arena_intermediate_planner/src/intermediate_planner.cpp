...
void InterPlanner::init(ros::NodeHandle &nh)
{
  ...
  bool intermediate_planner_active_mode;
  // why it failed to set this param?????????????? WTF
  node_.getParam("global_plannar/active_mode", intermediate_planner_active_mode);
  std::cout<<"\033[1;31m <JUST HINTs, NO WORRY,BE HAPPY>intermediate planner can not set this param,that means it wont't react to setting a new goal \033[0m\n"<<std::endl;
  std::cout<<"intermediate_planner_active_mode:"<<intermediate_planner_active_mode<<std::endl;
  ...
  // subscriber
  if (intermediate_planner_active_mode){
