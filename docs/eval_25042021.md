...
###Note: 
- for the run script it's not necessary to input the right scene, since the simulation environment already loaded a scene
- warnings like these can be ignored (it occurs because the simulation already spawned obstacles)
```
