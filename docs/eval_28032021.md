...
scenario json files are located at folder, in which you can change linear velocity:
```
src/arena-rosnav/simulator_setup/scenarios/eval
```

1. [Terminal 2] **Wait until all the obstacles in simulation environment are loaded**, start the *time-space plan manager*:
```
