...
class StateProvider():
    ...
    def __init__(self):
        ...
        self.list_of_lists = [[0.46,9.21], [4.1, 10.6], [10.12, 11.17], [13.13, 8.1], [19.7, 9.2], [21.4, 5.2], [23, 1]]
        self.wps = []
        ...
        self._globalPlan_sub = rospy.Subscriber('/globalPlan', Path, self.cbglobalPlan)   #todo in plan_manager.cpp publish the global plan
    ...
    def cbRobotPosition(self,msg):
        ...
        # idx = round(len(self._globalPlan.poses)/100) # 9
        # i = 0 
        # if i < idx : 
        #     self.downsampledPlan.append(self._globalPlan.poses[i*100])
        # self.downsampledPlan.append(self._globalGoal)
        # wps = self._globalPlan.poses[0::idx] 
        # self.downsampledPlan.append(wps)
        # self.downsampledPlan.append(self._globalGoal)
        # print(len(self.downsampledPlan))
        ...
        if self.i < 20: 
            ...
            self.navgoal.pose.position.x = self.downsampledPlan[0].pose.position.x
            self.navgoal.pose.position.y = self.downsampledPlan[0].pose.position.y
        ...
        if self.distance2subgoal.x < 0.6:
            ...
            self.navgoal.pose.position.x = self.downsampledPlan[self.j].pose.position.x
            self.navgoal.pose.position.y = self.downsampledPlan[self.j].pose.position.y
            self.navgoal_pub.publish(self.navgoal)
            #print("test2")
            self.j += 1
    ...
    def cbglobalPlan(self,msg):
        ...
        print(len(self._globalPlan.poses))
        idx = round(len(self._globalPlan.poses)/100) # 9
        i = 0 
        while i < idx : 
            self.downsampledPlan.append(self._globalPlan.poses[i*100])
